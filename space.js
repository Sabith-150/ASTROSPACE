...
function showSuggestions() {
    ...
    const suggestions = planets.filter(p => p.name.toLowerCase().startsWith(searchInput));

    suggestions.forEach(planet => {
        const li = document.createElement("li");
        li.textContent = planet.name;
        li.addEventListener("click", () => {
            document.getElementById("inputsearch").value = planet.name; 
            suggestionBox.innerHTML = ""; 
...
function displayPlanetInfo(planet) {
    ...
    displayDiv.innerHTML = `
        ...
        <p><strong>Notable Features:</strong> ${planet.notableFeatures}</p>
    `;
}
